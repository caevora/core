...
local names = {"John", "Jane"}
local ages = {25, 30}
local combined = table.zip(names, ages)
print(combined)
-- Output: {{"John", 25}, {"Jane", 30}}

--Game Development
local items = {"Sword", "Shield", "Potion"}
print(table.element_of(items))  -- Output: (random item, e.g., "Potion")

--Configuration Management
Example: Add or override user-defined settings on top of defaults.
local defaultConfig = {volume = 50, resolution = "1080p"}
local userConfig = {volume = 70}
local finalConfig = table.add(defaultConfig, userConfig)
print(finalConfig)
-- Output: {volume = 70, resolution = "1080p"}

--Data Analysis
local numbers = {1, 2, 3, 4, 5}
local sum = table.reduce(numbers, function(acc, val) return acc + val end, 0)
print(sum)  -- Output: 15


--Custom Table Types
local weakTable = table.new_weak("v")  -- Weak values
weakTable["key"] = {data = "test"}
collectgarbage()
print(weakTable["key"])  -- Output: nil (value is garbage-collected)

-Data Cleanup
local duplicates = {1, 2, 2, 3, 3, 3}
local unique = table.uniq(duplicates)
print(unique)  -- Output: {1, 2, 3}


---Predicate-Based Filtering
local numbers = {1, 3, 5, 8, 10}
local index = table.find(numbers, function(v) return v > 5 end)
print(index)  -- Output: 4 (index of value 8)
